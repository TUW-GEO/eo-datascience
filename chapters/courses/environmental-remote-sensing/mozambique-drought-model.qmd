...
## Linear Regression with Ordinary Least Squares as an Exploration Tool
...
We can see that the model fit with CHIRPS precipitation as independent variable is limited. Now let's do the same for the second predefined relationship, where we translate ($\text{NDVI} = \alpha + \beta \text{SSM} + \epislon$) into an `statsmodels` formula and asses it's fit, like so:
...
armed with this new infomation we can return to our OLS regression. We now can simply use this lag information to shift `precip` and `surface_soil_moisture` and perform the `statsmodels` OLS regression on this new dataset. We can simply use pandas `shift` to achieve this.
