...
The dataset used is the `Harmonized Landsat and Sentinel-2 (HLS)` product. From this data, yearly mosaics were created, where each variable represents a specific quantile of original pixel values across the time dimension. Before the quantiles have been calculated, the clouds have been masked.
This data is used to train the machine learning model for forest classification.

The other data that is used in this notebook is the `geojson` file, which is containing the training and testing areas in the region of interest. This data is used to define the spatial extent of areas with known land cover types, which is crucial for training and evaluating the model.
...
feature_frame.columns = [_rename_col(col) for col in feature_frame.columns]  # type: ignore
```
...
y_pred = clf_rf.predict(feature_frame)
...
print(classification_report(y_true=y_test, y_pred=class_frame.loc[y_test.index, "RF"]))
```

Lastly the predicted forest coverage map can be saved to disk for further analysis and visualization.
The results for this specific usecase has already been computed and saved in the same place where the source data is located. In the next notebook we will explore the results and visualize the loss of forest cover over time.
