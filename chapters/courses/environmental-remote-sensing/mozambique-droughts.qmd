...
We exemplify this normalization step below. Here we can see two histograms for a simulated SSM dataset. The histogram on the right is still in the original "degree of saturation" units, whereas the graph on the left is given as the Z score. The value of the x axis of the right-hand figure can be translated as: "This point is so many standard deviations removed from the mean."
