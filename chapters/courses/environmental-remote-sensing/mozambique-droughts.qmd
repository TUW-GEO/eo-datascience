...
- [Earth Observation Data Science Cookbook](https://projectpythia.org/eo-datascience-cookbook/)
-
...
from data.src.colours import SSM_CMAP  # TU Wien SSM colour map
```
...
```{python}
df = pd.read_csv("data/ascat-6_25_ssm_monthly.csv", index_col="time", parse_dates=True)
...
```{python}
ts = pd.read_csv(
    "data/ascat-6_25_ssm_timeseries.csv", index_col="time", parse_dates=True
)
